 ...
 */
// Reassign JQuery to work with Prototype.js
var $j = jQuery.noConflict();
$j(document).ready(function($){
      $("#MainDockBarNav").magnifier({ magnification: 2.5, overlap: true, zIndex: 10, opacity: { min: 0.6, max: 1 }, baseline: -1  });
    });
...
function loadOtherFiles()
 { // spread it out, a little, speed things up
   loadScript("./scripts/combineJS.php?type=javascript&files=niftycube.js,behaviour.js,window.js,iCalendar.js,iGallery.js", null, "html");

   loadCSS("./css/combineCSS.php?type=css&files=niftyCorners.css,defaultWin.css,mac_os_x.css,nuncio.css,alphacube.css,spread.css");
 }
...
function delegateAjaxContentRequest(object, index)
 {
...
function load_app(object, index)
 { // get the caption from the field
   delegateAjaxContentRequest(object, index);
   var itemSelected = object.title;
...
function processAnyOnDemandJavascript()
 {
   ...
   if ( contentElement != null )
   {
      ...
      if ( title.indexOf("home") > -1 )
      {
        ...
        setTimeout('Nifty("p.rounderItem", "big");',75);
        setTimeout('Nifty("h5.rounderItem", "med");',75);
      ...
      {
        Behaviour.apply();
        setTimeout('Nifty("div.rounderItem", "big");',60);
...
function disableDockBar()
 {
   $j("#MainDockBarNav").magnifierDisable();
 }
function enableDockBar()
 {
   $j("#MainDockBarNav").magnifierEnable();
 }
